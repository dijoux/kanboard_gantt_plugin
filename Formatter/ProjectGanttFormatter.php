    ...
    public function format()
    {
        ...
        foreach ($projects as $project_) {
            ...
            $bars[] = array(
                ...
                'popup' => '<div class="title">' . $project['name'] . '<br>' . $project['owner_name'] . '</div><div class="subtitle"><div class="subtitle-info"><strong>' . t("Start") . ':</strong> ' . date(t("Y-m-d"), $start) . '<br><strong>' . t("End") . ':</strong> ' . date(t("Y-m-d"), $end) . '<br>' . t("%s%% completed", ((!empty($project['nb_tasks'])) ? round($project['nb_closed_tasks'] / $project['nb_tasks'] * 100, 0) : '0')) . '</div><div class="subtitle-button"><span class="button">' . $this->helper->url->link('<i class="fa fa-fw fa-th"></i> ' . t('Board'), 'BoardViewController', 'show', array('project_id' => $project['id'])) . '</span>&nbsp;<span class="button">' . $this->helper->url->link('<i class="fa fa-fw fa-sliders"></i> ' . t('Gantt'), 'TaskGanttController', 'show', array('project_id' => $project['id'], 'plugin' => 'Gantt')) . '</span></div><div class="markdown">' . $this->helper->text->markdown($project['description']) . '</div></div>',
            );
        }
